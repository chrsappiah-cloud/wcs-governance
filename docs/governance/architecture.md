...
- A minimal set of claims (such as `org_role` and `is_staff`) is added to support coarse-grained routing; fine-grained access remains in the database.
...
- GitHub Actions workflows can be used to automate periodic evidence export, producing CI artifacts or pull requests with updated documentation.
...
This appendix maps the architecture above to the live codebase in [`chrsappiah-cloud/wcs-governance`](https://github.com/chrsappiah-cloud/wcs-governance).

| Component | Repository |
|-----------|------------|
| Governance console + migrations | `wcs-governance` |
| Public marketing site (current) | `wcs-full` |
| iOS applications | Separate repos; shared Supabase project |

**Enforcement layers:** JWT claims (`is_staff`, `org_role`) → `requireStaff()` → `requirePermission(key, scope)` → RLS → audit triggers.

**Migrations** (apply in order): `001_core_governance.sql`, `002_permissions_seed.sql`, `003_rls.sql`, `004_audit_triggers.sql`.

**R&D export:** server action `createRDEvidence`; CLI `npm run export:rd`; GitHub Action `.github/workflows/rd-report.yml`.

**PDF exports:** `npm run export:governance-pdf` → `docs/governance/exports/architecture.pdf`; R&D packs → `docs/rd-evidence/*.pdf`.

**Related docs:** [roles-and-scopes.md](./roles-and-scopes.md) · [R&D evidence](../rd-evidence/README.md) · [Integration with wcs-full](../INTEGRATION.md)
